...
router.get('/', async (req, res) => {
  ...
  try {
    const [
      ...
      supabaseAdmin
        .from('bookings')
        .select('booking_code, customer_name, service_type, scheduled_at, service_total, booking_status, created_at'),
      supabaseAdmin
        .from('orders')
        .select('order_code, customer_name, category, order_date, items, total, status, created_at'),
      supabaseAdmin
        .from('profiles')
    ...
    const bookingRows = (bookings || []).map((row) => ({
      ...row,
      eventDate: parseDate(row.scheduled_at || row.created_at),
