...
alter table cafe.inventory_items
...
begin
  if not exists (
    ...
    where conname = 'inventory_items_item_type_chk'
      and conrelid = 'cafe.inventory_items'::regclass
  ) then
    alter table cafe.inventory_items
...
create table if not exists cafe.inventory_recipe_lines (
  id uuid primary key default gen_random_uuid(),
  finished_item_id uuid not null references cafe.inventory_items(id) on delete cascade,
  raw_item_id uuid not null references cafe.inventory_items(id) on delete restrict,
...
drop trigger if exists trg_inventory_recipe_lines_updated_at on cafe.inventory_recipe_lines;
create trigger trg_inventory_recipe_lines_updated_at
before update on cafe.inventory_recipe_lines
for each row execute procedure cafe.set_updated_at();

create index if not exists idx_inventory_recipe_lines_finished_item_id
  on cafe.inventory_recipe_lines(finished_item_id);
create index if not exists idx_inventory_recipe_lines_raw_item_id
  on cafe.inventory_recipe_lines(raw_item_id);

create table if not exists cafe.inventory_stock_movements (
  id uuid primary key default gen_random_uuid(),
  inventory_item_id uuid not null references cafe.inventory_items(id) on delete cascade,
...
alter table cafe.inventory_stock_movements
  add column if not exists reversal_of_movement_id uuid references cafe.inventory_stock_movements(id) on delete set null,
  add column if not exists reversed_by_movement_id uuid references cafe.inventory_stock_movements(id) on delete set null,
...
create index if not exists idx_inventory_stock_movements_item_id
  on cafe.inventory_stock_movements(inventory_item_id);
create index if not exists idx_inventory_stock_movements_type_created_at
  on cafe.inventory_stock_movements(movement_type, created_at desc);
create index if not exists idx_inventory_stock_movements_created_at
  on cafe.inventory_stock_movements(created_at desc);
create index if not exists idx_inventory_stock_movements_reference_id
  on cafe.inventory_stock_movements(reference_id);
create index if not exists idx_inventory_stock_movements_reversal_of
  on cafe.inventory_stock_movements(reversal_of_movement_id);

create table if not exists cafe.menu_item_inventory_recipe_lines (
  id uuid primary key default gen_random_uuid(),
  menu_item_id uuid not null references cafe.menu_items(id) on delete cascade,
  inventory_item_id uuid not null references cafe.inventory_items(id) on delete restrict,
  quantity_required numeric(12,3) not null check (quantity_required > 0),
  unit text,
...
alter table cafe.menu_item_inventory_recipe_lines
  add column if not exists display_quantity text;

drop trigger if exists trg_menu_item_inventory_recipe_lines_updated_at on cafe.menu_item_inventory_recipe_lines;
create trigger trg_menu_item_inventory_recipe_lines_updated_at
before update on cafe.menu_item_inventory_recipe_lines
for each row execute procedure cafe.set_updated_at();

create index if not exists idx_menu_item_inventory_recipe_lines_menu_item_id
  on cafe.menu_item_inventory_recipe_lines(menu_item_id);
create index if not exists idx_menu_item_inventory_recipe_lines_inventory_item_id
  on cafe.menu_item_inventory_recipe_lines(inventory_item_id);

create or replace function cafe.apply_inventory_stock_movement(
...
as $$
declare
  v_item cafe.inventory_items%rowtype;
  v_saved_item cafe.inventory_items%rowtype;
  v_movement cafe.inventory_stock_movements%rowtype;
...
begin
  if auth.uid() is not null and not cafe.is_owner_or_staff() then
  ...
  insert into cafe.inventory_stock_movements (
    inventory_item_id,
    movement_type,
    quantity_delta,
    quantity_before,
...
create or replace function cafe.produce_inventory_finished_product(
...
as $$
declare
  v_finished cafe.inventory_items%rowtype;
  v_saved_item cafe.inventory_items%rowtype;
  v_raw cafe.inventory_items%rowtype;
  v_movement cafe.inventory_stock_movements%rowtype;
  v_line cafe.inventory_recipe_lines%rowtype;
...
begin
  ...
  select count(*)
    into v_recipe_count
  from cafe.inventory_recipe_lines
...
create or replace function cafe.deduct_menu_item_inventory_for_order_item()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
declare
  v_line cafe.menu_item_inventory_recipe_lines%rowtype;
  v_item cafe.inventory_items%rowtype;
  v_saved_item cafe.inventory_items%rowtype;
  v_required numeric(12,3);
...
begin
  ...
  for v_line in
    select *
    from cafe.menu_item_inventory_recipe_lines
...
drop trigger if exists trg_order_items_deduct_menu_inventory on cafe.order_items;
create trigger trg_order_items_deduct_menu_inventory
after insert on cafe.order_items
for each row execute procedure cafe.deduct_menu_item_inventory_for_order_item();

create or replace function cafe.restore_menu_item_inventory_for_order(
  p_order_id uuid,
...
declare
  ...
  v_item cafe.inventory_items%rowtype;
  v_saved_item cafe.inventory_items%rowtype;
  v_undo_movement cafe.inventory_stock_movements%rowtype;
...
begin
  ...
  for v_movement in
    select *
    from cafe.inventory_stock_movements
  ...
  loop
    ...
    update cafe.inventory_stock_movements
    set
...
create or replace function cafe.restore_menu_inventory_on_order_status()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  ...
  then
    perform cafe.restore_menu_item_inventory_for_order(
      new.id,
...
drop trigger if exists trg_orders_restore_menu_inventory on cafe.orders;
create trigger trg_orders_restore_menu_inventory
after update of status on cafe.orders
for each row execute procedure cafe.restore_menu_inventory_on_order_status();

alter table cafe.inventory_recipe_lines enable row level security;
alter table cafe.inventory_stock_movements enable row level security;
alter table cafe.menu_item_inventory_recipe_lines enable row level security;

grant select, insert, update, delete on table cafe.inventory_recipe_lines to authenticated;
grant select, insert, update on table cafe.inventory_stock_movements to authenticated;
grant select, insert, update, delete on table cafe.menu_item_inventory_recipe_lines to authenticated;
grant execute on function cafe.apply_inventory_stock_movement(uuid, text, numeric, text, text, jsonb, uuid) to authenticated;
grant execute on function cafe.produce_inventory_finished_product(uuid, numeric, text, text) to authenticated;
grant execute on function cafe.restore_menu_item_inventory_for_order(uuid, text) to authenticated;

drop policy if exists "inventory_recipe_lines_manage_owner_staff" on cafe.inventory_recipe_lines;
drop policy if exists "inventory_recipe_lines_read_owner_staff" on cafe.inventory_recipe_lines;
drop policy if exists "inventory_recipe_lines_insert_owner_staff" on cafe.inventory_recipe_lines;
drop policy if exists "inventory_recipe_lines_update_owner_staff" on cafe.inventory_recipe_lines;
drop policy if exists "inventory_recipe_lines_delete_owner_staff" on cafe.inventory_recipe_lines;
create policy "inventory_recipe_lines_read_owner_staff"
on cafe.inventory_recipe_lines for select
using (cafe.is_owner_or_staff());

create policy "inventory_recipe_lines_insert_owner_staff"
on cafe.inventory_recipe_lines for insert
with check (cafe.is_owner_or_staff());

create policy "inventory_recipe_lines_update_owner_staff"
on cafe.inventory_recipe_lines for update
using (cafe.is_owner_or_staff())
with check (cafe.is_owner_or_staff());

create policy "inventory_recipe_lines_delete_owner_staff"
on cafe.inventory_recipe_lines for delete
using (cafe.is_owner_or_staff());

drop policy if exists "inventory_stock_movements_read_owner_staff" on cafe.inventory_stock_movements;
create policy "inventory_stock_movements_read_owner_staff"
on cafe.inventory_stock_movements for select
using (cafe.is_owner_or_staff());

drop policy if exists "inventory_stock_movements_insert_owner_staff" on cafe.inventory_stock_movements;
create policy "inventory_stock_movements_insert_owner_staff"
on cafe.inventory_stock_movements for insert
with check (cafe.is_owner_or_staff());

drop policy if exists "inventory_stock_movements_update_owner" on cafe.inventory_stock_movements;
create policy "inventory_stock_movements_update_owner"
on cafe.inventory_stock_movements for update
using (cafe.is_owner())
with check (cafe.is_owner());

drop policy if exists "menu_item_inventory_recipe_lines_read_owner_staff" on cafe.menu_item_inventory_recipe_lines;
create policy "menu_item_inventory_recipe_lines_read_owner_staff"
on cafe.menu_item_inventory_recipe_lines for select
using (cafe.is_owner_or_staff());

drop policy if exists "menu_item_inventory_recipe_lines_insert_owner_staff" on cafe.menu_item_inventory_recipe_lines;
create policy "menu_item_inventory_recipe_lines_insert_owner_staff"
on cafe.menu_item_inventory_recipe_lines for insert
with check (cafe.is_owner_or_staff());

drop policy if exists "menu_item_inventory_recipe_lines_update_owner_staff" on cafe.menu_item_inventory_recipe_lines;
create policy "menu_item_inventory_recipe_lines_update_owner_staff"
on cafe.menu_item_inventory_recipe_lines for update
using (cafe.is_owner_or_staff())
with check (cafe.is_owner_or_staff());

drop policy if exists "menu_item_inventory_recipe_lines_delete_owner_staff" on cafe.menu_item_inventory_recipe_lines;
create policy "menu_item_inventory_recipe_lines_delete_owner_staff"
on cafe.menu_item_inventory_recipe_lines for delete
using (cafe.is_owner_or_staff());

drop trigger if exists trg_inventory_recipe_lines_activity on cafe.inventory_recipe_lines;
create trigger trg_inventory_recipe_lines_activity
after insert or update or delete on cafe.inventory_recipe_lines
for each row execute procedure cafe.log_table_activity();

drop trigger if exists trg_inventory_stock_movements_activity on cafe.inventory_stock_movements;
create trigger trg_inventory_stock_movements_activity
after insert or update or delete on cafe.inventory_stock_movements
for each row execute procedure cafe.log_table_activity();

drop trigger if exists trg_menu_item_inventory_recipe_lines_activity on cafe.menu_item_inventory_recipe_lines;
create trigger trg_menu_item_inventory_recipe_lines_activity
after insert or update or delete on cafe.menu_item_inventory_recipe_lines
for each row execute procedure cafe.log_table_activity();
