...
begin
  ...
  if p_order_type = 'delivery' then
    ...
    v_delivery_validation := cafe.validate_delivery_address(v_delivery_payload);
...
create or replace function cafe.get_guest_order_for_tracking(
  p_order_ref text,
  p_guest_phone_normalized text default null,
  p_guest_email text default null
)
...
begin
  ...
  return jsonb_build_object(
    'order', to_jsonb(v_order),
    'items', coalesce(
      (select jsonb_agg(to_jsonb(oi) order by oi.created_at, oi.id) from cafe.order_items oi where oi.order_id = v_order.id),
      '[]'::jsonb
    ),
    'history', coalesce(
      (select jsonb_agg(to_jsonb(h) order by h.changed_at) from cafe.order_status_history h where h.order_id = v_order.id),
...
create or replace function cafe.merge_guest_orders_into_customer(
  p_guest_phone_normalized text default null,
  p_guest_email text default null
)
returns jsonb
...
grant execute on function cafe.validate_delivery_address(jsonb) to anon, authenticated;
...
grant execute on function cafe.get_guest_order_for_tracking(text, text, text) to anon, authenticated;
grant execute on function cafe.merge_guest_orders_into_customer(text, text) to authenticated;
